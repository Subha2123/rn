...
import { setStudentToken } from '../api/token';
...
const StudentLoginScreen = () => {
    ...
    const validationSchema= Yup.object().shape({
       ...
       async function handleSubmit({email,password}){
         await axios.post('http://192.168.43.2:5647/api/student/login',{email,password})
         .then(async res=>{
            console.log(res.data) 
            if(res.data){
                await setStudentToken(res.data)
    ...
    return (
        ...
        <CustomIcon container1={styles.iconreg} title="Regsiter" icon="account-wrench" color="grey" size={30} onPress={()=>navigation.navigate("Student Register")}></CustomIcon>
        </View>
